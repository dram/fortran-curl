...
MODULES_DIR = modules
OBJECT_FILES = sources/curl.o tests/main.o
...
sources/curl.o: sources/curl.f90 ${CONSTANTS_FILE}

${CONSTANTS_FILE}: ${CONSTANTS_GENERATOR}
	$^ >$@

${CONSTANTS_GENERATOR}: sources/constants.c
	${CC} -Wall $(shell pkg-config --cflags libcurl) -o $@ $^

%.o: %.f90
	${CC} -std=f2008ts -Wall -J ${MODULES_DIR} -o $@ -c $<

${PROGRAM}: ${OBJECT_FILES}
	${CC} -o $@ $^ -lgfortran $(shell pkg-config --libs libcurl)
